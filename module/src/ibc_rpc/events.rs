...
pub async fn subscribe_ibc_event(client: Client<MyConfig>) -> Result<Vec<IbcEvent>> {
    ...
    'outer: while let Some(events) = event_sub.next().await {
        ...
        let events: Vec<RawEventDetails> = events.iter_raw().map(|event| event.unwrap()).collect();

        if inner_convert_event(events.clone(), "Ibc").is_empty() {
            continue;
        } else {
            for event in events {
                ...
                match variant.as_str() {
                    ...
                    "OpenInitConnection" => {
                        ...
                        let counterparty_connection_id =
                            event.3.map(|val| val.to_ibc_connection_id());
                    ...
                    //         )
                    //             .unwrap();
                    //     println!("In call_ibc: [subscribe_events] >> ExtrinsicSuccess ");
                    //
