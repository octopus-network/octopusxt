...
use crate::call_ibc::get_block_header;
...
pub async fn build_proof(
    ...
    block_number: u32,
) -> Result<(Vec<help::ValidatorMerkleProof>, MmrProof), Box<dyn std::error::Error>> {
    let api = src_client
        .clone()
    ...
    // covert to string
    let authorities: Vec<String> = authorities
        .into_iter()
        .map(|authority| {
            format!(
                "{}",
        ...
        .collect();
    println!("eth_addresss : {:?}", authorities);

    // build validator set merkler proofs by authories
    let validator_merkle_proofs = build_validator_merkle_proof(authorities).unwrap();
    println!(
        "info validator_merkle_proofs : {:#?}",
        validator_merkle_proofs
    );

    // generae mmr leaf and mmr leaf proof
    // target_height = signed_commitment.commitment.block_number-1
    // need make sure mmr_generate_proof index is u64
    let target_height = block_number - 1;
    let params = &[to_json_value(target_height)?];
    let mmr_proof: pallet_mmr_rpc::LeafProof<String> = api
        .client
        .rpc()
        .client
    ...
    println!("info generate_proof : {:?}", mmr_proof);
    // let (_, mmr_leaf, mmr_proof) = generate_proof;
    let mmr_leaf = mmr::MmrLeaf::decode(&mut &mmr_proof.leaf.0[..]).unwrap();
    let mmr_leaf_proof = mmr::MmrLeafProof::decode(&mut &mmr_proof.proof.0[..]).unwrap();
    println!("mmr_leaf of generate_proof : {:?}", mmr_leaf);
    println!("mmr_proof of generate_proof : {:?}", mmr_leaf_proof);

    //return  validator_merkle_proofs and mmr_proof
    Ok((
        validator_merkle_proofs,
        MmrProof {
...
pub fn build_validator_merkle_proof(
    ...
    for l in 0..eth_addresss.len() {
        ...
        println!("generate address proof  = {:?}", proof.proof);
        println!(
            "generate address number_of_leaves = {}",
            proof.number_of_leaves
        );
        println!("generate address leaf_index = {}", proof.leaf_index);
        println!("generate address leaf = {}", hex::encode(&proof.leaf));
        assert_eq!(
...
mod tests {
    ...
    #[tokio::test]
    async fn test_build_proof() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // get block hash
        let api = client.clone().to_runtime_api::<RuntimeApi<DefaultConfig>>();
        let block_hash = api.client.rpc().block_hash(block_number).await?;
        let proof = build_proof(client, block_hash, 100).await?;
        println!("build proof = {:?}", proof);
        Ok(())
    }

    #[tokio::test]
    async fn test_start_update_clien_state() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let target_client = ClientBuilder::new()
            ...
            .build::<ibc_node::DefaultConfig>()
            .await?;
        start_update_clien_state(src_client, target_client).await?;
        Ok(())
    }
